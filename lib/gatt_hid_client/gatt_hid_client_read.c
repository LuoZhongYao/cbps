     ...
     GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM_T* cfm)
{
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_read_ccd_cfm() status [%x] report id [%x] \n", hid_client_status, message->report_id));
...
/****************************************************************************/
static bool hid_client_read_ccd(GHIDC_T *const hid_client, gatt_hid_ccd_type mode)
{
    bool ret_val = FALSE;
    ...
    if(VALIDATE_HID_CLIENT(hid_client))
    {
        ...
        for(count = 0;count < num_ccd ;count++)
        {
            ...
            if(hid_client->ccd_handles[count].ccd_type == mode)
            {
                ...
                GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_read_ccd() report_count= %d,handle = %x\n",count, ccd_handle));
                {
                    /* Make internal message */
                    MAKE_HID_CLIENT_MESSAGE(HID_CLIENT_INTERNAL_MSG_READ_CCD);
                    message->ccd_handle = ccd_handle;
                    MessageSendConditionally((Task)&hid_client->lib_task, HID_CLIENT_INTERNAL_MSG_READ_CCD, message, &hid_client->pending_request);
                }
                ret_val = TRUE;
            }
        }        
...
{  
    if(VALIDATE_HID_CLIENT(hid_client))
    {
        GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_read_ccd_handle() handle = %x\n",msg->ccd_handle));
...
/****************************************************************************/
gatt_hid_client_status GattHidReadCCD(GHIDC_T *const hid_client, gatt_hid_ccd_type mode)
{
    gatt_hid_client_status   retval =  gatt_hid_client_status_invalid_param;
    ...
    if(VALIDATE_HID_CLIENT(hid_client))
    {
        ...
        if((mode == gatt_hid_boot_ccd) && (!hid_client->boot_mode_supported))
        {
            return gatt_hid_client_status_not_supported;
        }

        if (hid_client_read_ccd(hid_client, mode))
        {
            retval = gatt_hid_client_status_initiated;
        }
    }
