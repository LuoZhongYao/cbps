                ...
                case MUSIC_SIGNAL_DETECTOR_STATUS_RESP:
                    ...
                    */

                    {
                        ...
                        {
                            MAKE_AUDIO_MESSAGE(AUDIO_PLUGIN_AUDIO_SIGNAL_DETECT_MSG);
                            message->signal_detected = signal;
                            MessageSend(DECODER->app_task, AUDIO_PLUGIN_AUDIO_SIGNAL_DETECT_MSG, message);
                        }

                        csrA2dpDecoderPluginOutputMute(multi_channel_group_all, (signal ? AUDIO_MUTE_DISABLE : AUDIO_MUTE_ENABLE));
                    }
                    break;
...
static void sendDspSpdifConfigMessages(A2dpPluginConnectParams* codecData)
{
    ...
    
    {
        ...
        Source l_source = StreamAudioSource(AUDIO_HARDWARE_SPDIF, AUDIO_INSTANCE_0, SPDIF_CHANNEL_A );
...
{
    KalimbaSendMessage(MESSAGE_VM_TWS_COMPATIBILITY_MODE, buffer_level_required, 0, 0, 0);
}
...
static void csrA2dpSetHardwareGainDelayed(multi_channel_group_t group, i16 master_gain, u16 delay_ms)
{
    ...
    {
        CsrMultiChanGainSetHardware(group, master_gain, NULL);
    }
}
...
DESCRIPTION
    ...
    Return TRUE if message sent successfully or in case where no actions taken.
    Return FALSE if firmware failed to sent the message.
*/
static bool muteOutput(audio_plugin_mch_group_t group, AUDIO_MUTE_STATE_T state)
{
	bool result = TRUE;
        ...
        (DECODER->mute_state[audio_mute_group_aux] != state))
    {
        ...
        if(KalimbaSendLongMessage(MESSAGE_MULTI_CHANNEL_MUTE_AUX, sizeof(AUDIO_PLUGIN_MULTI_CHANNEL_AUX_MUTE_MSG_T), (const u16*)&mute_msg))
        {
			...
			DECODER->mute_state[audio_mute_group_aux] = state;
			(void) MessageCancelAll((TaskData *)DECODER->task, AUDIO_PLUGIN_SET_SOFT_MUTE_MSG);
        }
        else
		{ 
			result = FALSE;
...
void CsrA2dpDecoderPluginSetSoftMute(AUDIO_PLUGIN_SET_SOFT_MUTE_MSG_T* message)
{
    ...
    else
    {
        ...
        if((!muteOutput(multi_channel_group_main, new_main_state)) ||
        		(!muteOutput(multi_channel_group_aux, new_aux_state)))
        {
    		...
    		/*MessageSendLater((TaskData *)DECODER->task, AUDIO_PLUGIN_SET_SOFT_MUTE_MSG , message , ALLOW_FULL_DSP_BUFFER_DELAY_FOR_SOFT_MUTE);*/
    		MessageSend((TaskData *)DECODER->task, AUDIO_PLUGIN_SET_SOFT_MUTE_MSG , message);
        }
