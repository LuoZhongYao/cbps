...
typedef struct
{
    ...
    TP_BD_ADDR_T            tp_addrt;
    uint8_t                 io_capability; /*
                                             Apart from IO capabilities,
                                             this field's MSB is utilized to
                                             indicate application flexibility
                                             on MITM requirement when MITM
                                             flag is set in
                                             authentication_requirements
                                             field.
                                             At present, MITM flexibility
                                             is supported only for LE.
                                            */
    uint8_t                 oob_data_present;
    uint8_t                 authentication_requirements;
    uint8_t                 *oob_hash_c;
 ...
 *
 *  Request to generate and send back local OOB data.
 *  This can be used for both le as well as BREDR
 *  For now this call would be effective only for host SMP based keys.
 *
 *  -------------------------------------------------------------------------*/
typedef struct
{
 ...
 *
 *  Confirmation of keys being generated. Once the keys are deleted 
 ...
 *
 *  Event indicating that keys are generated. This might happen auitomatically 
 ...
 *
 *  Notification that the local key was deleted and next time around pairing
 *  would require a fresh pair of keys. The application might pre-generate them 
 *  or it would be generated runtime when needed.
 *
 *  -------------------------------------------------------------------------*/
 typedef struct
{
...
/*---------------------------------------------------------------------------*
 *  DM_SM_OOB_LOCAL_DATA_CFM
 *
 *  Notification of out of band data of the local device for low energy 
 *  secure connections. Out of band data includes local confirm value 
 *  and local random value.
 *
 *  oob_commit_c will contain 128 bit confirm value.
 *  oob_commit_r will contain 128 bit random value.
 *
 *  For tp_type as LE_ACL:
 *  oob_hash_c = DM_SM_OOB_DATA_ID_xx|hash_xx|
                 DM_SM_OOB_DATA_ID_yy|hash_yy|DM_SM_OOB_DATA_ID_NONE
 *  oob_rand_r = DM_SM_OOB_DATA_ID_xx|rand_xx|
                 DM_SM_OOB_DATA_ID_yy|rand_yy|DM_SM_OOB_DATA_ID_NONE
 *  oob_data_present shall be populated using the macros explained in 
 ...
 *
 *  For LE slave : this API invokes a L2CAP signalling procedure to
 *  update the parameters. Note that receiving 
 *  DM_BLE_UPDATE_CONNECTION_PARAMETERS_CFM_T could only mean that parameters
 *  are accepted/rejected between L2CAP layers.
 *  If accepted, confirmation does not mean that parametes has been applied
 ...
 * 
 *  For LE master : it invokes an HCI command to trigger Link Layer
 *  procedure to update the parameters. In return, applications will receive
