...
*/
static void processInputEvent(inputEvent_t event, u16 mask)
{
...
void notifyInputManager(inputEvent_t event, u16 mask, u16 id)
{
    ...
    switch(event)
    {
        ...
        case inputEventDown:
        {
            ...
            {
                INPUT_LOGD("IN: Event[%x] mask[%x] : ", event, mask);
        ...
        {
            INPUT_LOGD("IN: Event[%x] mask[%x] : ", event, theSink.rundata->inputManager.down_mask);
