...
bool deviceManagerGetAttributes(sink_attributes* attributes, const bdaddr* dev_addr)
{
    ...
    if (attributes)
    {
        DEV_DEBUG(("prof:0x%02X route:%u,%u HfpVol:%d a2dpVol:%d\n",
               attributes->profiles, 
               attributes->master_routing_mode,
               attributes->slave_routing_mode,
               attributes->hfp.volume, 
...
{
    DEV_DEBUG(("DEV: StoreAttribs - profiles %d, route %u,%u peer %d, features %x hfp_vol %d, a2dp_vol %d sub_trim %d gatt %d batt_l %d batt_r %d batt_p %d\n", 
               attributes->profiles, 
               attributes->master_routing_mode, 
			   attributes->slave_routing_mode,
               attributes->peer_device, 
