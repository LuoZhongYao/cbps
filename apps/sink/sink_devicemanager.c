...
void deviceManagerUpdateAttributesWithDeviceDefaults(sink_attributes* attributes, sink_remote_device_type device_type)
{
    ...
    LOGD("getDefaultAttrib - prof:0x%02X route:%u,%u, peer %d, features %x hfp_vol %d, a2dp_vol %d sub_trim %d gatt %d batt_l %d batt_r %d batt_p %d\n",
               attributes->profiles,
...
bool deviceManagerGetAttributes(sink_attributes* attributes, const bdaddr* dev_addr)
{
    ...
    u8 size = (attributes ? sizeof(sink_attributes) : 0);
    /* Attempt to retrieve attributes from PS */
    bool ok = ConnectionSmGetAttributeNow(0, dev_addr, size, (u8*)attributes);

#ifdef DEBUG_DEV
    LOGD("GetAttribs %04X %02X %06lX ",
               dev_addr->nap,
               dev_addr->uap,
               dev_addr->lap);
    if (attributes)
    {
        LOGD("prof:0x%02X route:%u,%u HfpVol:%d a2dpVol:%d\n",
               attributes->profiles,
               attributes->master_routing_mode,
               attributes->slave_routing_mode,
               attributes->hfp.volume,
...
{
    return ConnectionSmGetIndexedAttributeNowReq(0, index, sizeof(sink_attributes), (u8*)attributes, dev_addr);
}
...
bool deviceManagerGetAddrFromProfile(sink_link_type profile, typed_bdaddr* dev_addr)
{
    ...
    while (validPdlIndex)
    {
    	validPdlIndex = ConnectionSmGetIndexedAttributeNowReq(0, pdlIndex, sizeof(sink_attributes), (u8*)&attributes, dev_addr);
        ...
        else if(!validPdlIndex)
        {
            ...
            validPdlIndex = ConnectionSmGetIndexedAttributeNowReq(0, pdlIndex, 0, (u8*)NULL, dev_addr);
...
{
    LOGD("StoreAttribs - profiles %d, route %u,%u peer %d, features %x hfp_vol %d, a2dp_vol %d sub_trim %d gatt %d batt_l %d batt_r %d batt_p %d\n",
               attributes->profiles,
               attributes->master_routing_mode,
			   attributes->slave_routing_mode,
               attributes->peer_device,
    ...
    ConnectionSmPutAttribute(0, dev_addr, sizeof(sink_attributes), (u8*)attributes);
}
...
void deviceManagerStoreDefaultAttributes(const bdaddr* dev_addr, sink_remote_device_type device_type)
{
    ...
    LOGD("StoreDefaultAttribs - dev[%x] profiles %d\n", device_type, attributes.profiles);
...
void deviceManagerUpdateAttributes(const bdaddr* bd_addr, sink_link_type link_type, hfp_link_priority hfp_priority, a2dp_link_priority a2dp_priority)
{
    ...
    else if(link_type == sink_swat)
    {
        ...
        LOGD("DelayUpdateAttributes - type %d profiles %d, SUB Vol idx %d\n",link_type, update->attributes.profiles, update->attributes.sub.sub_trim_idx);
    }
#endif

    LOGD("DelayUpdateAttributes type:%d prof:0x%02X route:%u,%u hfp_vol:%d, a2dp_vol:%d clock_mismatch:%d ver:%x features:%x device:%x\n",
               link_type,
    ...
    MessageSendConditionally(&theSink.task, EventSysUpdateAttributes, update, (const u16 *)AudioBusyPtr());
}
...
void deviceManagerDelayedUpdateAttributes(EVENT_UPDATE_ATTRIBUTES_T* update)
{
    ...
    else if (update->attributes.profiles == sink_swat)
    {
        ...
        LOGD("UpdateAttributesOld - profiles %d, SWAT Vol Idx %d\n", attributes.profiles, attributes.sub.sub_trim_idx);
        LOGD("UpdateAttributesOld - profiles %d, SWAT Vol Idx %d\n", attributes.profiles, new_attributes.sub.sub_trim_idx);
    }
#endif

    LOGD("UpdateAttr old prof:0x%02X route:%u,%u hfp_vol:%d, a2dp_vol:%d clock_mismatch:%d ver:%x features:%x device:%x\n",
               attributes.profiles,
               attributes.master_routing_mode,
               attributes.slave_routing_mode,
               attributes.hfp.volume,
    ...
    LOGD("UpdateAttr new prof:0x%02X route:%u,%u hfp_vol:%d, a2dp_vol:%d clock_mismatch:%d ver:%x features:%x device:%x\n",
...
u8 deviceManagerSetPriority(const bdaddr* dev_addr)
{
    ...
    if(deviceManagerNumConnectedDevs() > 1)
    {
        ...
        LOGD("Update MRU - two devices connected\n");
        ...
        {
            LOGD("Update MRU - two devices two profiles connected - primary device\n");
            ListId = 0;
        ...
        {
            LOGD("Update MRU - two devices two profiles connected - secondary device\n");
    ...
    else
    {
        ...
        LOGD("Update MRU - primary device\n");
        ConnectionSmUpdateMruDevice(dev_addr);
...
conn_mask deviceManagerProfilesConnected(const bdaddr * bd_addr)
{
    ...
    LOGD("connected %04X %02X %06lX profiles 0x%02X\n",
               bd_addr->nap,
               bd_addr->uap,
               bd_addr->lap,
               result);
...
conn_mask deviceManagerGetProfilesConnected(void)
{
    ...
    LOGD("get connected profiles 0x%02X\n", result);
...
bool deviceManagerCanConnect(void)
{
    ...
    */
    if(!theSink.MultipointEnable)
    {
        if((deviceManagerGetProfileAddr(conn_hfp_pri, &dev_addr1)) &&
            !(deviceManagerGetProfileAddr(conn_a2dp_pri, &dev_addr2)))
            {
                ...
                {
                    LOGD("No multipoint, allow the device to connect next profile\n");
                    return TRUE;
                }
                else
                {
                    LOGD("No multipoint, next profile is a different device, reject\n");
                    return FALSE;
                }
            }
    }
    ...
    {
        LOGD("Multipoint and not at max, allow the device to connect\n");
        return TRUE;
    }
    /* We can't accept any more connections */
    LOGD("No more connections\n");
    return FALSE;
}
