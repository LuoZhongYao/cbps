 ...
 */
static void BMCheckForButtonMatch ( u32 pButtonMask , ButtonsTime_t  pDuration  )  ;

static void BMCheckForButtonPatternMatch ( u32 pButtonMask ) ;
...
void buttonManagerAddMapping ( event_config_type * event_config, u8 index )
{
    ...
    if ( lInputEvent )
    {
        ...
        lInputEvent->ButtonMaskVC = (u32)((u32)((u32)event_config->state_mask & 0xC000 ) >>14);
        ...
        {
            /* add input mask bit */
            lButtonsTask->gPerformInputEdgeCheck |= (u32)((lInputEvent->ButtonMaskLS | (u32)(lInputEvent->ButtonMaskVC) << VREG_PIN));
        }
        /* otherwise must be a level check */
        else if(lInputEvent->Duration > B_INVALID)
        {
            lButtonsTask->gPerformInputLevelCheck |= (u32)((lInputEvent->ButtonMaskLS | (u32)(lInputEvent->ButtonMaskVC) << VREG_PIN));
        }

        LOGD("Add Mapping: Event[%x] Duration[%x] Input Mask[0x%lx] Level Check[0x%lx]\n", lInputEvent->Event
             , lInputEvent->Duration
             , (u32)((lInputEvent->ButtonMaskLS | (u32)(lInputEvent->ButtonMaskVC) << VREG_PIN))
    ...
    if (pButtonsTask->gButtonPatterns[lPatternIndex].EventToSend == B_INVALID )
    {
        ...
        for (lButtonIndex = 0 ; lButtonIndex < BM_NUM_BUTTONS_PER_MATCH_PATTERN ; lButtonIndex++)
        {
            ...
            if(pButtonsToMatch[lButtonIndex])
                pattern = (((u32)1)<<(pButtonsToMatch[lButtonIndex]-1));
...
static bool BMCheckButtonLock ( MessageId id )
{
    ...
    u16 statemask = ( (1 << deviceOutgoingCallEstablish) |
    ...
    switch(id)
    {
        ...
        case EventUsrChargerDisconnected:
            LOGD("Event bypasses button lock [%x]\n" , id);
            return FALSE;
        default:
            LOGD("Button Locked [%x]\n" , id);
            return TRUE;
    }
    ...
    {       /*Each Entry*/
        for (lEvIndex = 0 ; lEvIndex < BM_EVENTS_PER_CONF_BLOCK ; lEvIndex ++)
        {
            ...
            if ( lButtonEvent != NULL)
            {
               ...
               lMask = (u32)(lButtonEvent->ButtonMaskLS | (u32)(lButtonEvent->ButtonMaskVC) << VREG_PIN);
