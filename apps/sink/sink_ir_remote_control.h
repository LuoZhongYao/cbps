...
typedef struct
{
    u16              addr;   /* Address to identify the RC the button press was generated by */
    u16              mask;   /* Identifies which button is currently pressed down */
...
typedef struct
{
    ...
    u16 remote_address;      /* The address of the remote control that sent the IR code (an Infrared remote may use different addresses for different buttons, so need to ensure the code was actually sent from the known RC) */
...
typedef struct
{
    ...
    u16                  max_learning_codes;       /* The maximum number of IR codes the application can learn */
    u16                  learning_mode_timeout;    /* Failsafe for turning off the IR learning mode (ms) */
    u16                  learning_mode_reminder;   /* A reminder tone can be generated to indicate learning mode is active, this will be repeated every timeout (ms) */
    u16                  ir_pio;                   /* The PIO interfacing the InfraRed hardware */
...
typedef struct
{
    ...
    u16              size_lookup_table;      /* The number of entries in the configured lookup table (DOES NOT include learning mode lookup table) */
    irRcConfig_t        *config;                /* The config data for the IR remote control feature */
    
    /* IR learning mode */
    u16                 num_learnt_codes;    /* The number of IR codes that have been learnt */
    irLookupTableConfig_t  *learnt_codes;       /* Points to the memory area used to store the learnt codes (if any exist) */
    u16                 learn_this_input_id; /* When in learning mode, this is the input ID to learn */
    
    /* Runtime data */
    u16              button_mask;            /* Used to keep track of which button is held down (if any) */    
