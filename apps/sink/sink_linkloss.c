...
static bool getA2dpDeviceToProtect(bdaddr *bd_addr, u16 *a2dp_index);
static bool getBdAddrNonRoutedA2dpDevice(bdaddr *bd_addr, u16 *a2dp_index);
...
static void updateNonPeerA2dpManagement(u16 a2dp_index);
#ifdef ENABLE_PEER
    static void updatePeerA2dpManagement(u16 a2dp_index);
    static bool isPeerStreamingAsSource(u16 a2dp_index);
#endif
static bool isHfpConnected(u16 a2dp_index);
...
{
    u16 non_routed_source_a2dp_index;
    bdaddr non_routed_source_bd_addr;

    LOGD("linklossProtectStreaming protection %u\n", new_state);
    ...
    case linkloss_stream_protection_on:
        {
            if(getA2dpDeviceToProtect(&non_routed_source_bd_addr, &non_routed_source_a2dp_index))
            {
                ...
                LOGD("non_routed_idx %u protection_state 0x%x\n",
...
static bool getA2dpDeviceToProtect(bdaddr *bd_addr, u16 *a2dp_index)
{
    ...
    */
    LOGD("getA2dpDeviceToProtect - is_peer %u peer_idx %u is_master %u local_bdaddr > peer_bddadr %u state %u\n",
        a2dpGetPeerIndex(&peer_index), peer_index, peerIsTwsMaster(),
        peerCompareBdAddr(&theSink.local_bd_addr, &theSink.a2dp_link_data->bd_addr[peer_index]),
...
static bool getBdAddrNonRoutedA2dpDevice(bdaddr *bd_addr, u16 *a2dp_index)
{
    u16 a2dp_index_routed_source;
    u16 a2dp_index_non_routed_source;
        ...
        && getA2dpIndexFromSink(theSink.routed_audio, &a2dp_index_routed_source))
    {
        ...
        {
            LOGD("getBdAddrNonRoutedSource: non-routed index = %u\n", a2dp_index_non_routed_source);
...
{
    LOGD("linklossUpdateManagement protection 0x%x\n", theSink.stream_protection_state);
...
static void updateHfpManagement(const bdaddr *bd_addr , bool enable_management)
{
    LOGD("updateHfpManagement %u hfp index %u\n" ,
...
static void updateA2dpManagement(const bdaddr *bd_addr , bool enable_management)
{
    ...
    LOGD("updateA2dpManagement = %u , index = %u\n" , enable_management , a2dp_index);
...
static void updatePeerA2dpManagement(u16 a2dp_index)
{
    ...
    {   /* Relaying audio to a Slave Peer - don't manage linkloss */
        LOGD("updatePeerA2dpManagement: Index = %u a2dp manage linkloss = FALSE\n", a2dp_index);
        A2dpDeviceManageLinkloss(theSink.a2dp_link_data->device_id[a2dp_index], FALSE);
    }
    else
    {   /* Either no audio relay or acting as a Slave Peer */
        LOGD("updatePeerA2dpManagement: Index = %u a2dp  manage linkloss = TRUE\n", a2dp_index);
...
static bool isPeerStreamingAsSource(u16 a2dp_index)
{
    ...
    LOGD("isPeerStreamingAsSource index %u dev_id %u stream_id %u state %u role %u playing %u\n",
...
static void updateNonPeerA2dpManagement(u16 a2dp_index)
{
    ...
    {   /* HFP library will manage link loss */
        LOGD("updateNonPeerA2dpManagement: Id = %u (non-peer), a2dp manage linkloss = FALSE\n", a2dp_index);
        A2dpDeviceManageLinkloss(theSink.a2dp_link_data->device_id[a2dp_index], FALSE);
    }
    else
    {   /* No HFP connection, so A2DP library manages link loss */
        LOGD("updateNonPeerA2dpManagement: Id = %u (non-peer) a2dp manage linkloss = TRUE\n", a2dp_index);
        A2dpDeviceManageLinkloss(theSink.a2dp_link_data->device_id[a2dp_index], TRUE);
    }
}
...
static bool isHfpConnected(u16 a2dp_index)
{
    ...
    if( (HfpLinkGetBdaddr(hfp_primary_link, &HfpAddr) &&
        ...
        (HfpLinkGetBdaddr(hfp_secondary_link, &HfpAddr) &&
         BdaddrIsSame(&theSink.a2dp_link_data->bd_addr[a2dp_index], &HfpAddr)) )
    {
        LOGD("isHfpConnected: Index = %u (non-peer),  connected \n", a2dp_index);
        return TRUE;
    }

    LOGD("isHfpConnected: Index = %u (non-peer),  not connected \n", a2dp_index);
    return FALSE;
}
...
{
    LOGD("linklossSendLinkLossTone [%x:%x:%lx]\n",
...
{
    LOGD("linklossCancelLinkLossTone\n");
...
bool linklossIsStreamProtected(const bdaddr *bd_addr)
{
    ...
    LOGD("linklossIsStreamProtected idx %u protection 0x%x return %u\n",
...
void linklossResetStreamProtection(u16 a2dp_index)
{
    ...
    LOGD("linklossResetStreamProtection idx %u protection_state 0x%x\n",
        a2dp_index, theSink.stream_protection_state);
}
