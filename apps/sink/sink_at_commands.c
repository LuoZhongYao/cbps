...
static const u8 battery_scaling_2[] = {0,1,3,5,7,9};
...
void sinkHandleUnrecognisedATCmd(HFP_UNRECOGNISED_AT_CMD_IND_T *ind)
{
    ...
    LOGD("HFP_UNRECOGNISED_AT_CMD_IND\n" );
    ...
    if(!theSink.conf3)
    {
        LOGD("AT: Unrecognised AT Cmd -  no config\n" );
        return;
    }
...
void sinkSendATCmd( u16 at_id )
{
    ...
    if (count == at_id)
    {
        ...
        unsigned char * string_to_send = mallocPanic (strlen(search_string)+1) ;
...
void ATCommandPlayEvent ( sinkEvents_t id )
{
    ...
    for (i =0 ; i < MAX_AT_COMMANDS_TO_SEND ; i++ )
    {
        ...
        {
            LOGD("AT: Ev [%x] AT CMD [%d]" , id , theSink.conf3->gEventATCommands[i].at_cmd );
        ...
        switch ( theSink.conf3->gEventATCommands[i].event )
        {
            case EventSysGasGauge0:
                ...
                {
                    LOGD("AT: Ev Gas Gauge n [%x] AT CMD [%d]" , id , theSink.conf3->gEventATCommands[i].at_cmd );
                    sinkSendATCmd ( theSink.conf3->gEventATCommands[i].at_cmd ) ;
                }
            break ;
            case EventSysChargerGasGauge0:
                ...
                {
                    LOGD("AT: Ev Charger Gas Gauge n [%x] AT CMD [%d]" , id , theSink.conf3->gEventATCommands[i].at_cmd );
                    sinkSendATCmd ( theSink.conf3->gEventATCommands[i].at_cmd ) ;
                }
            break ;
            default:
