...
void powerManagerConfig(const power_config* config, const power_pmu_temp_mon_config* pmu_mon_config)
{
    ...
    if ((theSink.conf1 != NULL) && (config != NULL))
    {
        ...
        PsFullRetrieve(PSKEY_CHARGER_CALC_VTERM, &theSink.rundata->battery_limits.max_battery_v, sizeof(u16));
...
{
    LOGD("PM: Charger has been connected\n");
...
{
    LOGD("PM: Charger has been disconnected\n");
...
*/
static void powerManagerHandleVbatNormal(u8 level)
{
    bool low_batt = powerManagerIsVbatLow();
    LOGD("PM: Battery Normal %d\n", level);
...
static void powerManagerHandleVbat(voltage_reading vbat, battery_level_source source)
{
    ...
    LOGD("PM: Battery Voltage 0x%02X (%dmV)\n", vbat.level, vbat.voltage);
...
void handlePowerMessage( Task task, MessageId id, Message message )
{
    switch(id)
    {
        ...
        case POWER_BATTERY_VOLTAGE_IND:
        {
            ...
            LOGD("PM: POWER_BATTERY_VOLTAGE_IND\n");
        ...
        {
            LOGD("PM: POWER_CHARGER_VOLTAGE_IND\n");
        ...
        case POWER_BATTERY_TEMPERATURE_IND:
        {
            ...
            LOGD("PM: POWER_BATTERY_TEMPERATURE_IND\n");
        ...
        case POWER_CHARGER_STATE_IND:
        {
            ...
            LOGD("PM: POWER_CHARGER_STATE_IND\n");
        ...
        default :
            LOGD("PM: Unhandled Battery msg[%x]\n", id);
        break ;
    }
}
...
{
    LOGD("PM: Battery Management %s\n", theSink.lbipmEnable ? "Enabled" : "Disabled");
    LOGD("PM: Using %s Level ", powerManagerIsChargerConnected() ? "Chg" : "Batt");
...
bool powerManagerChargerSetup(voltage_reading* vthm)
{
    ...
    switch(setting.termination.type)
    {
        ...
        default:
            ...
            LOGD("PM: Termination Voltage Unchanged\n");
        break;
    }
...
u16 powerManagerBatteryLevelAsPercentage(void)
{
    ...
    u16 voltage = powerManagerReadVbatVoltage();
    ...
    if (theSink.rundata->battery_limits.max_battery_v > theSink.rundata->battery_limits.min_battery_v)
    {
        ...
        {
            battery_level = (u16)(((u32)(voltage - theSink.rundata->battery_limits.min_battery_v)  * (u32)100) / (u32)(theSink.rundata->battery_limits.max_battery_v - theSink.rundata->battery_limits.min_battery_v));
        }
    }
