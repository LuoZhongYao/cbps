...
#define HI(n) ((u8)((n) >> 8))
#define LO(n) ((u8)(n))
static u16 calculateVolume (u16 gain, u16 scale, u16 range)
{
...
**************************************************************************/
static u8 getCodecIncludeMask (void)
{
    u8 include_mask;
    ...
    if (theSink.a2dp_link_data)
    {
        ...
        for (i = 0; i<MAX_A2DP_CONNECTIONS; i++)
        {
            ...
            {   /* Found a connected device */
                u16 device_id = theSink.a2dp_link_data->device_id[i];
                u16 stream_id = theSink.a2dp_link_data->stream_id[i];

                LOGD("...... dev:%u str:%u state:%u\n", device_id, stream_id, A2dpMediaGetState(device_id, stream_id));
...
{
    LOGD("findCurrentStreamingA2dpSource\n");
...
static void reconnectAvSource(void)
{
    ...
    if(a2dpGetSourceIndex(&av_id))
    {
        ...
        {
            LOGD("reconnectAvSource because AG seid = 0x%x\n", seid);
...
{
    u16 local_optional_codecs;
    u16 remote_optional_codecs;
    u16 compatible_optional_codecs;
    u16 index = 0;

    if(peerGetLocalSupportedCodecs(&local_optional_codecs) && peerGetRemoteSupportedCodecs(&remote_optional_codecs))
    {
        compatible_optional_codecs = local_optional_codecs & remote_optional_codecs;

        LOGD("local_optional_codecs = 0x%x\n", local_optional_codecs);
        LOGD("remote_optional_codecs = 0x%x\n", remote_optional_codecs);
        LOGD("compatible_optional_codecs = 0x%x\n", compatible_optional_codecs);
        ...
        if(compatible_optional_codecs != local_optional_codecs)
        {
            while(optional_codec_seids[index].codec_seid != INVALID_SEID)
            {
                ...
                {
                    LOGD("disableIncompatibleOptionalCodecs seid = 0x%x\n", optional_codec_seids[index].codec_seid);
...
static void enableOptionalCodecs(void)
{
    ...
    if(peerGetLocalSupportedCodecs(&local_optional_codecs))
    {
        while(optional_codec_seids[index].codec_seid != INVALID_SEID)
        {
            ...
            {
                LOGD("enableOptionalCodecs seid = 0x%x\n", optional_codec_seids[index].codec_seid);
...
**************************************************************************/
static a2dp_codec_settings * getCodecSettings (u8 seid)
{
    u16 i;
...
**************************************************************************/
static void modifyCodecSettings (a2dp_codec_settings * codec_settings, u16 rate)
{
    u8 * codec_caps = NULL;
    u16 codec_caps_size;
    ...
    if(codec_settings == NULL)
{
    ...
    LOGD("modifyCodecSettings   codec_caps=0x%X   size=%u   rate=%u\n",(u16)codec_caps,codec_caps_size,rate);
    ...
    {
        u16 category_size = codec_caps[1] + 2;
    ...
    {   /* Media Codec service category located */
        ...
        LOGD("Found AVDTP_SERVICE_MEDIA_CODEC, attempting to modify caps...\n");
        LOGD("new_caps=0x%X   codec_caps[1]=%u  codec_caps[3]=%u   new_caps[1]=%u  new_caps[3]=%u\n", (u16)new_codec_caps, codec_caps[1], codec_caps[3], new_codec_caps[1], new_codec_caps[3]);
...
static void handleA2dpCodecConfigureIndFromPeer (A2DP_CODEC_CONFIGURE_IND_T* ind)
{
    ...
    u8* conf_caps = NULL;
    u16 size_conf_caps = 0;
    u8 *peer_caps = ind->codec_service_caps;
    u16 size_peer_caps = ind->size_codec_service_caps;

    LOGD("handleA2dpCodecConfigureIndFromPeer dev:%u seid:0x%X\n", ind->device_id, ind->local_seid);

    switch (current_source)
    {
        case RELAY_SOURCE_NULL:
            LOGD("... Streaming Av Src not found\n");
            break;
        ...
        case RELAY_SOURCE_A2DP:
            ...
            {
            u16 device_id = theSink.a2dp_link_data->device_id[priority];
            u16 stream_id = theSink.a2dp_link_data->stream_id[priority];
    ...
    if (codec_settings)
    {
        ...
        {   /* Source and sink seids do not use matching codec */
            LOGD("... Non matching codecs dev:%u seid:0x%X\n",ind->device_id, ind->local_seid);
...
**************************************************************************/
static bool openPeerStream (u16 Id, u8 base_seid)
{
    LOGD("openPeerStream Dev=%u base_seid=%02X",Id, base_seid);
	...
	if (base_seid)
	{
        ...
        LOGD("%02X, ", seid_list[seid_list_size-1]);
#endif
#ifdef PEER_AS
...
bool a2dpIssuePeerOpenRequest (void)
{
    ...
    {
        u8 seid = INVALID_SEID;
        u16 av_id;
        RelaySource current_source = theSink.peer.current_source;

        peerClaimRelay(TRUE);

        LOGD("issuePeerOpenRequest peer=%u  av=%u\n",peer_id,current_source);
...
bool a2dpIssuePeerCloseRequest (void)
{
    ...
    if (a2dpGetPeerIndex(&peer_id))
    {
        LOGD("issuePeerCloseRequest peer=%u\n",peer_id);
...
bool a2dpIssuePeerSuspendRequest (void)
{
    ...
    if (a2dpGetPeerIndex(&peer_id))
    {
        LOGD("issuePeerSuspendRequest peer=%u\n",peer_id);
...
bool a2dpIssuePeerStartResponse (void)
{
    ...
    if (a2dpGetPeerIndex(&peer_id))
    {
        LOGD("issuePeerStartResponse peer=%u\n",peer_id);
...
{
    LOGD("a2dpSetPlayingState id %u playing %u\n", id, playing);
    ...
    if ( a2dpGetPeerIndex(&peer_id) )
    {
        ...
        for_all_avrcp(avrcp_id)
        {
                ...
                (BdaddrIsSame(&theSink.a2dp_link_data->bd_addr[peer_id], &theSink.avrcp_link_data->bd_addr[avrcp_id])))
            {
                ...
                routing_modes[0] = (u8)slave_routing_mode;
                routing_modes[1] = (u8)master_routing_mode;

                sinkAvrcpVendorUniquePassthroughRequest( avrcp_id, AVRCP_PEER_CMD_TWS_AUDIO_ROUTING, sizeof(routing_modes), (const u8 *)routing_modes );
                break;
            }
        }
#endif
...
/* Only certain TWS use cases require us to attempt to pause source devices */
    ...
    LOGD("A2dp: a2dpPauseNonRoutedSource\n");
      ...
      )
    {
        ...
        {
            LOGD("A2dp: SuspendState - Stop playing Id = %d \n",id);
        ...
        {
            LOGD("A2dp: stopping avrcpIndex=%u\n", avrcpIndex);
            a2dpAvrcpStop(avrcpIndex);
        }
    }
#endif /* ENABLE_PEER */
...
void InitA2dp(void)
{
    ...
    {
        u8 codec_include_mask = getCodecIncludeMask();
        ...
        for (i=0; i<NUM_SEPS; i++)
        {
            if (codecList[i].config && (codecList[i].include_mask & codec_include_mask))
            {
                if (codecList[i].enable_bit==SBC_CODEC_BIT || (theSink.features.A2dpOptionalCodecsEnabled & (1<<codecList[i].enable_bit)))
                {
                    ...
                    LOGD("INIT: Codec included (seid=0x%X)\n",codecList[i].config->seid);
                }
            }
        }
...
#ifdef INCLUDE_A2DP_EXTRA_CODECS
       theSink.a2dp_link_data->local_peer_optional_codecs = theSink.features.A2dpOptionalCodecsEnabled;
#endif
...
/*            LOGD("A2dp: getA2dpStreamData - state=%u sink=0x%X\n",*state, (u16)*sink); */
        }
    }
}
...
**************************************************************************/
static bool openStream (u16 Id, u8 seid)
{
...
void sinkA2dpSetLinkRole (Sink sink, hci_role role)
{
    ...
    LOGD("sinkA2dpSetLinkRole  sink=0x%X  role=%s\n", (uintptr_t)sink, (role == hci_role_master) ? "master" : "slave");
    ...
    if (getA2dpIndexFromSink(sink, &index))
    {
        ...
        {
            LOGD("    setting  link_role[%u] = LR_CURRENT_ROLE_MASTER\n",index);
            theSink.a2dp_link_data->link_role[index] = LR_CURRENT_ROLE_MASTER;
        }
        else
        {
            LOGD("    setting  link_role[%u] = LR_CURRENT_ROLE_SLAVE\n",index);
...
void sinkA2dpInitComplete(const A2DP_INIT_CFM_T *msg)
{
    ...
    {
        LOGD("A2DP Init Success\n");
    }
    else
    {
	    LOGD("A2DP Init Failed [Status %d]\n", msg->status);
        Panic();
    }
}
...
**************************************************************************/
static bool isUnconnectedA2dpDevice (const bdaddr *bd_addr, u16* idx)
{
    for_all_a2dp(*idx)
    {
...
**************************************************************************/
#ifdef ENABLE_PEER
static void getKnownPeerDeviceAttributes(const u16 priority , sink_attributes *attributes)
{
...
void issueA2dpSignallingConnectResponse(const bdaddr *bd_addr, bool accept)
{
    ...
    LOGD("issueA2dpSignallingConnectResponse\n");

    if (!theSink.a2dp_link_data)
    {
        LOGD("NULL a2dp_link_data\n");
        return;
    }

    if (!isUnconnectedA2dpDevice(bd_addr, &idx))
    {   /* We have no knowledge of this device */
        LOGD("device unknown or already connected!\n");
        return;
    }
...
void handleA2DPSignallingConnectInd(u16 DeviceId, bdaddr SrcAddr)
{
         ...
         ((!theSink.a2dp_link_data->connected[a2dp_primary]) || (!theSink.a2dp_link_data->connected[a2dp_secondary])) )
    {
        ...
        u16 priority = (!theSink.a2dp_link_data->connected[a2dp_primary]) ? a2dp_primary : a2dp_secondary;

        LOGD("Signalling Success, Device ID = %x\n",DeviceId);
        ...
        {   /* Determine remote device type before accepting connection */
            ...
            {   /* Peer credentials not requested */
                ...
                {   /* We are initiating a Peer session */
                    LOGD("Peer Device SDP record not requested, assuming Peer\n");
...
void handleA2DPSignallingConnected(a2dp_status_code status, u16 DeviceId, bdaddr SrcAddr, bool locally_initiated)
{
    ...
    {
        u32 delay = theSink.conf1->timeouts.SecondAGConnectDelayTime_s;
        ...
        {
            LOGD("Delaying next slc connect request\n");
            delay += A2DP_PEER_PDL_PAGING_DELAY;
        }
#endif
    ...
    if (status != a2dp_success)
    {
        ...
        LOGD("Signalling Failed device=%u [Status %d]\n", DeviceId, status);
    ...
    else
    {
        ...
        LOGD("handleA2DPSignallingConnected: Asking for role check\n");
           ...
           ((theSink.a2dp_link_data->connected[a2dp_secondary])&&(BdaddrIsSame(&SrcAddr, &theSink.a2dp_link_data->bd_addr[a2dp_secondary]))))
        {
            ...
            LOGD("Signalling Connected following link loss [Status %d]\n", status);
        }
        else
        {
            /* store the device_id for the new connection in the first available storage position */
            ...
            {
            	LOGD("Signalling Success, Primary ID = %x\n",DeviceId);
            ...
            {
            	LOGD("Signalling Success, Secondary ID = %x\n",DeviceId);
  	 ...
  	 /* Ensure the underlying ACL is encrypted */
        ...
        else
        {
            ...
            if(getA2dpIndex(DeviceId, (u16*)&priority))
            {
...
{
    LOGD("A2dp: connectA2dpStream[%u] delay=%u\n", priority, delay);
...
{
   	LOGD("A2dp: OpenInd DevId = %d, seid = 0x%X\n",DeviceId, seid);
           ...
           if ((theSink.a2dp_link_data->SuspendState[Id] == a2dp_local_suspended) && (theSink.a2dp_link_data->peer_device[Id] != remote_device_peer))
           {
                ...
                for_all_avrcp(i)
                {
                        ...
                        (BdaddrIsSame(&theSink.a2dp_link_data->bd_addr[Id], &theSink.avrcp_link_data->bd_addr[i])))
                    {
                        ...
                        LOGD("Open Success - suspended - avrcp play\n");
                        break;
                    }
                }
#endif
                ...
                if(!status_avrcp)
                {
                    A2dpMediaStartRequest(DeviceId, StreamId);
              		LOGD("Open Success - suspended - start streaming\n");
                }
...
**************************************************************************/
static void handleA2DPClose(u16 DeviceId, u16 StreamId, a2dp_status_code status)
{
    ...
    if((status == a2dp_success) || (status == a2dp_disconnect_link_loss))
    {
        ...
        LOGD("A2dp: Close DevId = %d, StreamId = %d status = %u\n",DeviceId,StreamId, status);
    ...
    {
        LOGD("A2dp: Close FAILED status = %d\n",status);
    }
}
...
void handleA2DPSignallingDisconnected(u16 DeviceId, a2dp_status_code status,  bdaddr SrcAddr)
{
    ...
    LOGD("SigDiscon DevId = %d status %u SrcAddr [%x:%x:%lx]\n",
    ...
    * Make sure this is not an HFP profile (A2DP instead). */
    ...
    if(getA2dpIndex(DeviceId, &Id))
    {
            ...
            if (peerObtainPairingMode(Id) == PEER_PAIRING_MODE_TEMPORARY)
            {
                ...
                *message = theSink.a2dp_link_data->bd_addr[Id];
                ...
                if (linklossIsStreamProtected(&theSink.a2dp_link_data->bd_addr[Id]) && (status == a2dp_disconnect_link_loss) && theSink.conf1->timeouts.A2dpLinkLossReconnectionTime_s)
                {
                    ...
                    LOGD("A2dp: Starting a link loss timer\n");
        ...
        {   /* Kick off reconnect now */
       	...
       	LOGD("A2dp: Sig Discon FAILED status = %d\n",status);
...
{
    LOGD("A2dp: handleA2DPLinklossReconnectCancel DevId = %d\n",DeviceId);
...
{
    u16 Id;
    LOGD("A2dp: handleA2DPSignallingLinkloss DevId = %d\n",DeviceId);
       ...
       /* update the link policy */
        ...
        {   /* Open ind from true AV source */
            ...
            {   /* Either no peer device connected or we have a peer that has control of the relay channel. In either case just accept the media stream */
                LOGD("Relay channel NOT available\n");
                LOGD("Send start resp to AV Source\n");
                A2dpMediaStartResponse(DeviceId, StreamId, TRUE);
            }
        }
    }
...
void handleA2DPStartStreaming(u16 DeviceId, u16 StreamId, a2dp_status_code status)
{
    ...
    if(status == a2dp_success)
    {
        ...
        LOGD("A2dp: StartStreaming DevId = %d, StreamId = %d\n",DeviceId,StreamId);
        /* find structure index of deviceId */
        if(getA2dpIndex(DeviceId, &Id))
        {
...
#endif
            {
               ...
               set EQ mode as it might be that the device has a SCO routed instead */
                ...
                {
                    LOGD("A2dp: Wrong sink Don't Set EQ mode = %d\n",theSink.a2dp_link_data->a2dp_audio_mode_params.music_mode_processing);
                }
            }
    ...
    {
       	LOGD("A2dp: StartStreaming FAILED status = %d\n",status);
#ifdef ENABLE_PEER
        {
            u16 Id;
...
void handleA2DPSuspendStreaming(u16 DeviceId, u16 StreamId, a2dp_status_code status)
{
    ...
    {
       	LOGD("A2dp: Suspend Failed= %x, try close DevId = %d, StreamId = %d\n",status,DeviceId,StreamId);
    ...
    else
    {
        ...
        LOGD("A2dp: Suspend Ok DevId = %d, StreamId = %d\n",DeviceId,StreamId);
...
#ifdef ENABLE_PEER
            LOGD("handleA2DPSuspendStreaming - cancel pause-suspend timeout a2dp_idx %u\n", Id);
            a2dpCancelPauseSuspendTimeout(Id);
#endif
...
void SuspendA2dpStream(a2dp_link_priority priority)
{
    if (theSink.a2dp_link_data && (theSink.a2dp_link_data->peer_device[priority] != remote_device_peer))
    {
        ...
        u16 i;
#endif
        LOGD("A2dp: Suspend A2DP Stream %x\n",priority);
...
void  a2dpSuspendNonRoutedStream(a2dp_link_priority priority)
{
    ...
    {
        LOGD("A2dp:  a2dpSuspendNonRoutedStream priority %x\n",priority);

        /* set the local suspend status indicator */
        a2dpSetSuspendState(priority, a2dp_remote_suspended);

        /* attempt to suspend stream if avrcp pause was not successful, if not successful then close it */
        LOGD("Sending suspend\n");
        if(!A2dpMediaSuspendRequest(theSink.a2dp_link_data->device_id[priority], theSink.a2dp_link_data->stream_id[priority]))
        {
            LOGD("Sending suspend failed\n");
        }
    ...
    for_all_a2dp(i)
    {
        ...
        if(theSink.a2dp_link_data->connected[i])
        {
            ...
            if(BdaddrIsSame(&theSink.a2dp_link_data->bd_addr[i], bd_addr))
            {
                ...
                u16 lOldVol = theSink.volume_levels->a2dp_volume[i].main_volume;
                u16 a2dp_vol_max = sinkVolumeGetGroupConfig(multi_channel_group_main).no_of_steps - 1;
...
void handleA2DPStoreClockMismatchRate(u16 clockMismatchRate)
{
    ...
    {
        LOGD("A2dp: store pri. clk mismatch = %x\n", clockMismatchRate);
    ...
    {
        LOGD("A2dp: store sec. clk mismatch = %x\n", clockMismatchRate);
        theSink.a2dp_link_data->clockMismatchRate[a2dp_secondary] = clockMismatchRate;
    }
    else
    {
        LOGD("A2dp: ERROR NO A2DP STREAM, clk mismatch = %x\n", clockMismatchRate);
    }
}
...
**************************************************************************/
void handleA2DPStoreCurrentEqBank (u16 currentEQ)
{
    u16 abs_eq = A2DP_MUSIC_PROCESSING_FULL_SET_EQ_BANK0 + currentEQ;

    LOGD("A2dp: Current EQ = %x, store = %x\n", currentEQ, abs_eq);
...
/*****************************/
	    ...
	    case A2DP_SIGNALLING_CONNECT_IND:
	        LOGD("A2DP_SIGNALLING_CHANNEL_CONNECT_IND : \n");
	    ...
	    case A2DP_SIGNALLING_CONNECT_CFM:
            LOGD("A2DP_SIGNALLING_CHANNEL_CONNECT_CFM : \n");
    	...
    	case A2DP_SIGNALLING_DISCONNECT_IND:
            LOGD("A2DP_SIGNALLING_CHANNEL_DISCONNECT_IND : \n");
...
/*************************/
        ...
        case A2DP_MEDIA_RECONFIGURE_CFM:
            LOGD("A2DP_MEDIA_RECONFIGURE_CFM : \n");
#ifdef ENABLE_PEER
...
/*************************/
        ...
        case A2DP_MEDIA_AV_SYNC_DELAY_UPDATED_IND:
            LOGD("A2DP_MEDIA_AV_SYNC_DELAY_UPDATED_IND : seid=0x%X delay=%u\n", ((A2DP_MEDIA_AV_SYNC_DELAY_UPDATED_IND_T*)message)->seid,
	    ...
	    case A2DP_ENCRYPTION_CHANGE_IND:
            LOGD("A2DP_ENCRYPTION_CHANGE_IND : \n");
		break;

        case A2DP_LINKLOSS_RECONNECT_CANCEL_IND:
            LOGD("A2DP_LINKLOSS_RECONNECT_CANCEL_IND : \n");
            handleA2DPLinklossReconnectCancel(((A2DP_LINKLOSS_RECONNECT_CANCEL_IND_T*)message)->device_id);
        break;

        default:
	    	LOGD("A2DP UNHANDLED MSG: 0x%x\n",id);
        break;
    }
}
...
{
    u16 device_id = theSink.a2dp_link_data->device_id[a2dp_link];
    u16 stream_id = theSink.a2dp_link_data->stream_id[a2dp_link];
...
{
        u16 peerIndex = 0;
        bdaddr addr = *(bdaddr *)message;
        LOGD("EventSysRemovePeerTempPairing\n");
...
void a2dpStartPauseSuspendTimeout(a2dp_link_priority priority)
{
    ...
    LOGD("a2dpStartPauseSuspendTimeout idx %u\n", priority);
...
{
    LOGD("a2dpCancelPauseSuspendTimeout idx %u\n", priority);
